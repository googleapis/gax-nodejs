...
extend(gax, require('./page_streaming'));
 ...
 * @returns {Promise} - The promise which resolves to the response object.
 *   The promise has a method named "cancel" which cancels the ongoing API call.
 *   Or this resolves to an object representing [google.longrunning.ListOperationsResponse]{@link external:"google.longrunning.ListOperationsResponse"}
 ...
 * api.listOperations(request).then(function(resources) {
 ...
 *     if (nextPageToken) {
 *         // fetch the next page.
 *         api.listOperations(request, {autoPaginate: false, pageToken: nextPageToken}, callback);
 *     }
 * }
 * api.listOperations(request, {autoPaginate: false}, callback);
 */
...
/**
 * Stream version of {@link listOperations}.
 *
 * This is the same method as listOperations but returns a NodeJS Stream object.
 * The callback for 'data' event will be called everytime a new item arrives.
 *
 * @param {Object} request
 *   The request object that will be sent.
 * @param {string} request.name
 ...
 * });
 */
OperationsApi.prototype.listOperationsStream = function(request, options) {
  if (options === undefined) {
    options = {};
  }
