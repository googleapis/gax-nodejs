...
import {
  ...
  RequestType,
  CancellableStream,
} from '../../src/apitypes.js';
import {createApiCall} from '../../src/createApiCall.js';
import {StreamingApiCaller} from '../../src/streamingCalls/streamingApiCaller.js';
import * as gax from '../../src/gax.js';
import {StreamDescriptor} from '../../src/streamingCalls/streamDescriptor.js';
import * as streaming from '../../src/streamingCalls/streaming.js';
import {APICallback} from '../../src/apitypes.js';
...
describe('streaming', () => {
  ...
  it('handles client streaming', done => {
    ...
    const s = apiCall({}, undefined, (err: unknown, response: unknown) => {
  ...
  it('retries using resumption request function ', done => {
    ...
    s.on('error', (err: {message: unknown}) => {
...
describe('handles server streaming retries in gax when gaxStreamingRetries is enabled', () => {
  ...
  it('server streaming call retries until exceeding timeout and surfaces underlying error', done => {
    ...
    call.on('error', (err: {code: unknown; message: unknown}) => {
      try {
        assert(err instanceof GoogleError);
        if (err.code !== 14) {
  ...
  it('does not retry when there is no shouldRetryFn and retryCodes is an empty array', done => {
    ...
    call.on('error', (err: {code: unknown}) => {
      assert(err instanceof GoogleError);
      if (err.code === 14) {
  ...
  it('allows custom CallOptions.retry settings with shouldRetryFn instead of retryCodes and new retry behavior', done => {
    sinon
      ...
      .callsFake((stream: any): undefined => {
  ...
  it('allows the user to pass a custom resumption strategy', done => {
    sinon
      ...
      .callsFake((opts: any): CancellableStream => {
...
describe('REST streaming apiCall return StreamArrayParser', () => {
  const protos_path = path.resolve(dirname, '..', 'fixtures', 'user.proto');
  ...
  it('forwards data, end event', done => {
    ...
    s.on('data', (data: {resources: Array<number>}) => {
