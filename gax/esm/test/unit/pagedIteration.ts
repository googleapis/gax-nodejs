...
import {PageDescriptor} from '../../src/paginationCalls/pageDescriptor.js';
import {APICallback, GaxCall, RequestType} from '../../src/apitypes.js';
...
describe('paged iteration', () => {
  ...
  it('retries on failure', done => {
    ...
    const apiCall = util.createApiCallTest(failingFunc, createOptions);
    apiCall({}, undefined)
      .then(resources => {
        assert(Array.isArray(resources));
        // @ts-ignore response type
  ...
  describe('stream conversion', () => {
    ...
    beforeEach(() => {
      spy = sinon.spy(func);
      apiCall = util.createApiCallTest(spy, createOptions);
    });
