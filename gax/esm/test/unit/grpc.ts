...
//@ts-ignore
import testJson from '../../test/fixtures/library.json' with {type: 'json'};
...
const grpcVersion = grpcModulePackageJson.version;
...
describe('grpc', () => {
  ...
  describe('loadProto', () => {
    ...
    const TEST_PATH = path.resolve(dirname, '..', '..', 'test', 'fixtures');
    ...
    it('should load the test file as JSON', () => {
      ...
      // test will fail anyway.
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const protos = grpcClient.loadProtoJSON(testJson as INamespace) as any;
      assert.strictEqual(
        typeof protos.google.example.library.v1.LibraryService,
        'function'
      );
    ...
    it('should load a common proto', () => {
      const nonExistentDir = path.join(dirname, 'nonexistent', 'dir');
    ...
    it('should cache the loaded JSON proto', () => {
      const proto1 = grpcClient.loadProtoJSON(testJson as INamespace);
      const proto2 = grpcClient.loadProtoJSON(testJson as INamespace);
      assert.strictEqual(proto1, proto2);
    });
  ...
  describe('GoogleProtoFilesRoot', () => {
    ...
    const NON_EXISTENT_FILE = path.join(dirname, 'does', 'not', 'exist.proto');
    ...
    describe('use with protobufjs load', () => {
      ...
      it('should load a test file', done => {
        protobuf
          ...
          .then((root: {lookup: (param: string) => any}) => {
