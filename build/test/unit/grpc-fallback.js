...
(0, mocha_1.describe)('loadProto', () => {
    ...
    (0, mocha_1.it)('should create a root object using loadProtoJSON', () => {
        // @ts-ignore incomplete options
        const gaxGrpc = new fallback_1.GrpcClient(opts);
    ...
    (0, mocha_1.it)('should cache root object using loadProtoJSON', () => {
        // @ts-ignore incomplete options
        const gaxGrpc = new fallback_1.GrpcClient(opts);
    ...
    (0, mocha_1.it)('should not cache root object using loadProtoJSON when asked', () => {
        // @ts-ignore incomplete options
        const gaxGrpc = new fallback_1.GrpcClient(opts);
...
(0, mocha_1.describe)('grpc-fallback', () => {
    ...
    (0, mocha_1.it)('should send grpc-web version in the header', () => {
        const gapicConfig = {
            interfaces: {
    ...
    (0, mocha_1.it)('should handle an API error', done => {
        const requestObject = { content: 'test-content' };
        ...
        const expectedMessage = '3 INVALID_ARGUMENT: One of content, or gcs_content_uri must be set.';
        const expectedError = {
            code: 3,
            details: [
                {
        ...
        gaxGrpc.createStub(echoService, stubOptions).then(echoStub => {
            echoStub.echo(requestObject, {}, {}, (err) => {
                ...
                assert.strictEqual(JSON.stringify(err.statusDetails), JSON.stringify(expectedError.details));
                done();
            });
        });
    });
    (0, mocha_1.it)('should handle a fetch error', done => {
        const requestObject = { content: 'test-content' };
        //@ts-ignore
        sinon.stub(nodeFetch, 'Promise').rejects(new Error('fetch error'));
        gaxGrpc.createStub(echoService, stubOptions).then(echoStub => {
            echoStub.echo(requestObject, {}, {}, (err) => {
                assert.strictEqual(err === null || err === void 0 ? void 0 : err.message, 'fetch error');
                done();
            });
        });
    });
    (0, mocha_1.it)('should promote ErrorInfo if exist in fallback-rest error', done => {
        const requestObject = { content: 'test-content' };
        ...
        gaxGrpc.createStub(echoService, stubOptions).then(echoStub => {
            echoStub.echo(requestObject, {}, {}, (err) => {
                var _a;
                assert(err instanceof src_1.GoogleError);
                assert.strictEqual(JSON.stringify((_a = err.statusDetails) === null || _a === void 0 ? void 0 : _a.length), JSON.stringify(serverError['error']['details'].length));
    ...
    (0, mocha_1.it)('should have close method', done => {
        // @ts-ignore
        sinon.stub(nodeFetch, 'Promise').returns(Promise.resolve({}));
