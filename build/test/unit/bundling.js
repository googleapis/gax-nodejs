...
(0, mocha_1.describe)('computeBundleId', () => {
    (0, mocha_1.describe)('computes the bundle identifier', () => {
        const testCases = [
            {
        ...
        testCases.forEach(t => {
            (0, mocha_1.it)(t.message, () => {
                assert.strictEqual((0, bundlingUtils_1.computeBundleId)(t.object, t.fields), t.want);
            });
        });
    });
    (0, mocha_1.describe)('returns undefined if failed', () => {
        const testCases = [
            {
        ...
        testCases.forEach(t => {
            (0, mocha_1.it)(t.message, () => {
                assert.strictEqual((0, bundlingUtils_1.computeBundleId)(t.object, t.fields), undefined);
            });
        });
    });
});
(0, mocha_1.describe)('deepCopyForResponse', () => {
    (0, mocha_1.it)('copies deeply', () => {
        const input = { foo: { bar: [1, 2] } };
        const output = (0, task_1.deepCopyForResponse)(input, null);
        assert.deepStrictEqual(output, input);
        assert.notStrictEqual(output.foo, input.foo);
        assert.notStrictEqual(output.foo.bar, input.foo.bar);
    });
    (0, mocha_1.it)('respects subresponseInfo', () => {
        const input = { foo: [1, 2, 3, 4], bar: { foo: [1, 2, 3, 4] } };
        const output = (0, task_1.deepCopyForResponse)(input, {
            field: 'foo',
            start: 0,
            end: 2,
        });
        ...
        const output2 = (0, task_1.deepCopyForResponse)(input, {
            field: 'foo',
            start: 2,
            end: 4,
        });
    ...
    (0, mocha_1.it)('deep copies special values', () => {
        class Copyable {
            constructor(id) {
                this.id = id;
            }
    ...
    (0, mocha_1.it)('ignores erroneous subresponseInfo', () => {
...
(0, mocha_1.describe)('Task', () => {
    ...
    (0, mocha_1.describe)('extend', () => {
        ...
        (0, mocha_1.describe)('increases the element count', () => {
            testCases.forEach(t => {
                (0, mocha_1.it)(t.message, () => {
                    const task = testTask();
                    const baseCount = task.getElementCount();
                    extendElements(task, t.data);
                    assert.strictEqual(task.getElementCount(), baseCount + t.want, t.message);
                });
            });
        });
        (0, mocha_1.describe)('increases the byte size', () => {
            ...
            testCases.forEach(t => {
                (0, mocha_1.it)(t.message, () => {
                    ...
                    extendElements(task, t.data);
                    assert.strictEqual(task.getRequestByteSize(), baseSize + t.want * sizePerData);
                });
            });
        });
    });
    (0, mocha_1.describe)('run', () => {
        ...
        (0, mocha_1.describe)('sends bundled elements', () => {
            testCases.forEach(t => {
                (0, mocha_1.it)(t.message, done => {
        ...
        (0, mocha_1.describe)('calls back with the subresponse fields', () => {
            testCases.forEach(t => {
                (0, mocha_1.it)(t.message, done => {
                    const apiCall = sinon.spy(createApiCall(t.expected));
                    const task = testTask(apiCall);
        ...
        (0, mocha_1.describe)('calls back with fail if API fails', () => {
    ...
    (0, mocha_1.it)('cancels existing data', done => {
        const apiCall = sinon.spy((resp, callback) => {
            callback(null, resp);
        });
        const task = testTask(apiCall);
    ...
    (0, mocha_1.it)('cancels ongoing API call', done => {
        const apiCall = sinon.spy((resp, callback) => {
    ...
    (0, mocha_1.it)('partially cancels ongoing API call', done => {
        const apiCall = sinon.spy((resp, callback) => {
            const timeoutId = setTimeout(() => {
                callback(null, resp);
            }, 100);
...
(0, mocha_1.describe)('Executor', () => {
    ...
    (0, mocha_1.it)('groups api calls by the id', () => {
    ...
    (0, mocha_1.it)('emits errors when the api call fails', done => {
        const executor = newExecutor({ delayThreshold: 10 });
    ...
    (0, mocha_1.it)('runs unbundleable tasks immediately', done => {
        const executor = newExecutor({ delayThreshold: 10 });
    ...
    (0, mocha_1.describe)('callback', () => {
        ...
        (0, mocha_1.it)("shouldn't block next event after cancellation", done => {
        ...
        (0, mocha_1.it)('distinguishes a running task and a scheduled one', done => {
            let counter = 0;
    ...
    (0, mocha_1.it)('respects element count', () => {
        const threshold = 5;
    ...
    (0, mocha_1.it)('respects bytes count', () => {
    ...
    (0, mocha_1.it)('respects element limit', done => {
        const threshold = 5;
        const limit = 7;
    ...
    (0, mocha_1.it)('respects bytes limit', done => {
        const unitSize = byteLength(1);
        const threshold = 5;
        const limit = 7;
        const executor = newExecutor({
    ...
    (0, mocha_1.it)('does not invoke runNow twice', done => {
        const threshold = 2;
        const executor = newExecutor({
            elementCountThreshold: threshold,
            delayThreshold: 10,
        ...
        setTimeout(() => {
            assert.strictEqual(spy.callCount, 1);
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            assert.strictEqual(executor._runNow.callCount, 1);
            done();
        }, 20);
    });
    (0, mocha_1.describe)('timer', () => {
        (0, mocha_1.it)('waits on the timer', done => {
        ...
        (0, mocha_1.it)('reschedules after timer', done => {
            const executor = newExecutor({ delayThreshold: 50 });
            const spy = sinon.spy(apiCall);
            const start = new Date().getTime();
...
(0, mocha_1.describe)('bundleable', () => {
    ...
    (0, mocha_1.it)('bundles requests', done => {
        ...
        const apiCall = (0, utils_1.createApiCall)(spy, settings);
    ...
    (0, mocha_1.it)('does not fail if bundle field is not set', done => {
        const spy = sinon.spy(func);
    ...
    (0, mocha_1.it)('suppresses bundling behavior by call options', done => {
        const spy = sinon.spy(func);
        let callbackCount = 0;
    ...
    (0, mocha_1.it)('cancels partially on bundling method', done => {
        const apiCall = (0, utils_1.createApiCall)(func, settings);
    ...
    (0, mocha_1.it)('properly processes camel case fields', done => {
        const descriptor = new bundleDescriptor_1.BundleDescriptor('data', ['log_name'], 'data', byteLength);
        const settings = {
            settings: { bundleOptions },
            descriptor,
        };
