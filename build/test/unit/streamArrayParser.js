...
function createRandomChunkReadableStream(data) {
    assert.notEqual(data, undefined);
    const stream = new stream_2.PassThrough({
        objectMode: true,
    });
...
(0, mocha_1.describe)('Parse REST stream array', () => {
    ...
    (0, mocha_1.it)('should successfully decode array of valid JSON', done => {
        ...
        const inStream = createRandomChunkReadableStream(JSON.stringify(expectedResults));
        inStream.on('data', d => {
            assert.notEqual(d, undefined);
        });
        const streamArrayParser = new streamArrayParser_1.StreamArrayParser(streamMethod);
        (0, stream_1.pipeline)(inStream, streamArrayParser, err => {
            if (err) {
        ...
        streamArrayParser.on('end', () => {
            for (const key in expectedResults) {
                const expect = (0, utils_1.toProtobufJSON)(User, expectedResults[key]);
                assert.strictEqual(JSON.stringify(results[key]), JSON.stringify(expect));
            }
            done();
        });
    });
    (0, mocha_1.it)('should assign default value if the service response is not valid protobuf specific JSON', done => {
        const expectedResults = [
            {
    ...
    (0, mocha_1.it)("should listen on error if input stream is not start '['", done => {
        const expectedResults = {};
        const inStream = createRandomChunkReadableStream(JSON.stringify(expectedResults));
        const streamArrayParser = new streamArrayParser_1.StreamArrayParser(streamMethod);
        (0, stream_1.pipeline)(inStream, streamArrayParser, err => {
